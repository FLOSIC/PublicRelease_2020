...
! ***************************************************************
!
  SUBROUTINE GETHOLD_MGGA(NEWIT,IFNCT,JFNCT,AI,AJ,HSUB)
!
...
!       REAL*8,ALLOCATABLE :: PISV(:,:)
       ...
       real*8  :: ALPI2, ALPJ2
       REAL*8  :: TM2, TM1, TIM1
       real*8  :: ENVI, VOLMGGA, GPI(3), GPJ(3), PI, PJ, GPJS, PJGPJ(3)
...
! mixin has both spins. multiplied by WMSH in overnum_libxc
         IPTR = (ISPN-1)*NMSH
!         VOL1(1,MPTS)=DVPOT(1,KPTS) *EXP(-ALPIONE*RISQ)
...
!
         GRADP( 2 ,1,MPTS,1)=1.0D0   !d/dx (x-Rx)
         GRADP( 3 ,2,MPTS,1)=1.0D0   !d/dy (y-Ry)
         GRADP( 4 ,3,MPTS,1)=1.0D0   !d/dz (z-Rz)
                    
         GRADP( 5 ,1,MPTS,1)=2*RI(1) !d/dx (x-Rx)^2
         GRADP( 6 ,2,MPTS,1)=2*RI(2) !d/dy (y-Ry)^2
         GRADP( 7 ,3,MPTS,1)=2*RI(3) !d/dz (z-Rz)^2
                    
         GRADP( 8 ,1,MPTS,1)=RI(2)   !d/dx (x-Rx)(y-Ry)
         GRADP( 8 ,2,MPTS,1)=RI(1)   !d/dy (x-Rx)(y-Ry)
         GRADP( 9 ,1,MPTS,1)=RI(3)   !d/dx (x-Rx)(z-Rz)
         GRADP( 9 ,3,MPTS,1)=RI(1)   !d/dz (x-Rx)(z-Rz)
         GRADP(10 ,2,MPTS,1)=RI(3)   !d/dy (y-Ry)(z-Rz)
         GRADP(10 ,3,MPTS,1)=RI(2)   !d/dz (y-Ry)(z-Rz)
         
         R(1, MPTS, 1)=RI(1) 
         R(2, MPTS, 1)=RI(2) 
         R(3, MPTS, 1)=RI(3) 
         
!         VOL1(5,MPTS)=POTDV(5,KPTS) *EXP(-ALPIONE*RISQ)
         VOL1(5,MPTS)=mixin(4,KPTS+IPTR) *EXP(-ALPIONE*RISQ)
...
!       
            GRADP( 2 ,1,IPTS,2)=1.0D0    !d/dx (x-Rx)
            GRADP( 3 ,2,IPTS,2)=1.0D0    !d/dy (y-Ry)
            GRADP( 4 ,3,IPTS,2)=1.0D0    !d/dz (z-Rz)
                       
            GRADP( 5 ,1,IPTS,2)=2*RJ(1)  !d/dx (x-Rx)^2
            GRADP( 6 ,2,IPTS,2)=2*RJ(2)  !d/dy (y-Ry)^2
            GRADP( 7 ,3,IPTS,2)=2*RJ(3)  !d/dz (z-Rz)^2
                       
            GRADP( 8 ,1,IPTS,2)=RJ(2)    !d/dx (x-Rx)(y-Ry)
            GRADP( 8 ,2,IPTS,2)=RJ(1)    !d/dy (x-Rx)(y-Ry)
            GRADP( 9 ,1,IPTS,2)=RJ(3)    !d/dx (x-Rx)(z-Rz)
            GRADP( 9 ,3,IPTS,2)=RJ(1)    !d/dz (x-Rx)(z-Rz)
            GRADP(10 ,2,IPTS,2)=RJ(3)    !d/dy (y-Ry)(z-Rz)
            GRADP(10 ,3,IPTS,2)=RJ(2)    !d/dz (y-Ry)(z-Rz)
...
!
             ...
             DO IPTS=1,MPTS
              IF (PTOT*ENV(IPTS) .GT. ZERO/100.0D0) THEN
               ...
               ENVI=ENV(IPTS)
               VOLMGGA=VOL1(5,IPTS)*ENVI
!unrolling k=1,3 (gradients) loop speed up by at least 2x.
...
! combined grad(phi_i) gga/mgga terms 
                PJGPJ(1)=PJ*VOL1(2,IPTS) + GPJ(1)*VOLMGGA 
                PJGPJ(2)=PJ*VOL1(3,IPTS) + GPJ(2)*VOLMGGA 
                PJGPJ(3)=PJ*VOL1(4,IPTS) + GPJ(3)*VOLMGGA 
! gga phi_i terms
                GPJS=(GPJ(1)*VOL1(2,IPTS)  &
                     +GPJ(2)*VOL1(3,IPTS)  &
                     +GPJ(3)*VOL1(4,IPTS)) *ENVI
...
!                       *VOL1(K+1,IPTS) *ENVI
! mgga 
!                      +GRADP(I,IPTS,K,1)*GRADP(J,IPTS,K,2) * VOLMGGA * ENVI
                  END DO !I
                 END DO !J
              END IF !PTOT
             END DO !IPTS
