...
!           IDELFX  : selects gradient expansion corrections
     ...
     &                              IINTERP, IDELFX)
      ...
      !       Exchange Energy[FD]
          eps_x = AX*den**(1.d0/3.d0)*fx!exlda*fx
     ...
     &      1.d0 /)
          integer, dimension(8), parameter :: f_x_e = (/
     &      7,6,5,4,3,2,1,0/)
     ...
     &       + gx*(ddel_fxdp + dh1xdp - ief*dh1xdp)
          dfxda = gx*((h0x - h1x)*diefda + ddel_fxda
     &      + dh1xda - ief*dh1xda)
...
!******************************************************************************
! CORRELATION SUBROUTINES from cscan.f
!******************************************************************************
...
!           IDELFX  : selects gradient expansion corrections
          ...
          real(8) :: alpha, dadd0, dadg0, dadt0, dadd1, dadg1
          real(8) ::  dadt1, oma, reg, dreg
     ...
     &        -1.915710236206d0, 0.516884468372d0, -0.051848879792d0/)
          ...
          !        Zeta
          zeta = min(max((d0 - d1)/dt, -0.99999999999990d0),
     &           0.99999999999990d0)
          ...
          !        Reduced density gradient
          s = gt/(2.d0*(3.d0*PI2)**(1.d0/3.d0)*dt**(4.d0/3.d0))
     ...
     &    - dtauwdg1/(tueg + ETA*tauw)
          ...
          if (IALPHA .eq. 1) then
              ...
              reg = alpha**3/(alpha**2 + A_REG)
              dreg = (alpha**4 + 3.d0*alpha**2*A_REG)/(alpha**2 + A_REG)**2
              dadd0 = dadd0*dreg
              dadd1 = dadd1*dreg
              dadg0 = dadg0*dreg
              dadg1 = dadg1*dreg
          ...
          dec1ds = 0.d0
          dec1dz = 0.d0
          call scan_ec1(rs, s, zeta, ec1, dec1drs,
     &                  dec1dz, dec1ds, IE_PARAMS_C, ETA, IDELEC)
     ...
     &         - (1.d0 + zeta)**(1.d0/3.d0))/3.d0
          ...
          gc_z = (1.d0 - 2.363d0*(dx_z - 1.d0))*(1.d0 - zeta**12)
          dgc_zdz = -(1.d0 - 2.363d0*(dx_z - 1.d0))*12.d0*zeta**11
          dgc_zdz = dgc_zdz - 2.363d0*ddx_zdz*(1.d0 - zeta**12)
     ...
     &  gc, dgcdz, phi3, dphi3dz, w1, dw1drs, dw1dz, GAMMA,
     &  del_y, ddel_ydrs, ddel_ydz, ddel_yds, IE_PARAMS_C, ETA)
          IMPLICIT NONE
     ...
     &            - (1.d0 + zeta)**(2.d0/3.d0))
          ...
          dt1drs = -t1*dw1drs/w1
          dt1dz = -del_f2*(w1*(phi3*dds_zdz
     &         + ds_z*dphi3dz) + ds_z*phi3*dw1dz)
     &         /(27.d0*GAMMA*(ds_z*phi3*w1)**2)
     ...
     &          0.880260d0, 0.496710d0, rs, alfm, dalfmdrs, dalfmdrsrs)
          ...
          F = ((1.d0 + zeta)**(4.d0/3.d0)
     &        + (1.d0 - zeta)**(4.d0/3.d0) - 2.d0)/GAM
          dFdz = -4.d0*((1.d0 - zeta)**(1.d0/3.d0)
     &        - (1.d0 + zeta)**(1.d0/3.d0))/(3.d0*GAM)

          eclda1 = EU*(1.d0 - F*z4) + EP*F*z4
     &             - ALFM*F*(1.d0 - z4)/FZZ
          declda1drs = (1.d0 - z4*F)*deudrs + z4*F*depdrs
     &              - (1.d0 - z4)*F*dalfmdrs/FZZ
          declda1dz = EU*(-4.d0*z3*F - z4*dFdz)
     &              + z4*EP*dFdz + 4.d0*z3*EP*F
